...
Scenario 2 – Port failure simulation (link DOWN then UP)

IMPORTANT – Load from Mininet CLI with globals():
    mininet> py exec(open('tests/test_scenarios.py').read(), globals())
    mininet> py run_scenario1(net)
    mininet> py run_scenario2(net)

Do NOT use exec() without globals() — the functions will be defined
in a temporary local scope and immediately lost.
"""
...
def run_scenario1(net):
    ...
    Tests:
      • h1 ↔ h2  (same switch, should have ~1ms RTT)
      • h1 ↔ h3  (cross switch, should have ~6-10ms RTT)
      • h2 ↔ h4  (cross switch)
...
def run_scenario2(net):
    """
    Scenario 2: Simulate port DOWN event on the s1-s2 inter-switch link.
    Tests:
      • Connectivity before failure (cross-switch should pass)
      • Port DOWN event detected by controller (check Terminal 1)
      • Same-switch ping still works during failure
      • Cross-switch ping fails during failure
    ...
    print("  ⚠️  Link s1-s2 brought DOWN. Watch Terminal 1 for ALERT.\n")
    time.sleep(3)

    # Step 3: Test during failure
    ...
    print("  ✅  Link s1-s2 restored. Watch Terminal 1 for RECOVERY.\n")
    time.sleep(3)
...
if __name__ == "__main__":
    print("This script must be loaded from the Mininet CLI.")
    print("")
    print("Usage:")
    print("  mininet> py exec(open('tests/test_scenarios.py').read(), globals())")
