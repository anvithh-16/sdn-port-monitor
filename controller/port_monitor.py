...
# ── Log directory: always relative to THIS file, not cwd ──────────────────────
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(LOG_DIR, "port_monitor.log")),
...
class PortMonitorController(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Statistics: {dpid: {port_no: {tx_bytes, rx_bytes, ...}}}
        self.port_stats = {}

        # Connected datapaths registry (populated on connect, cleared on disconnect)
        self.datapaths = {}
    ...
    def port_status_handler(self, ev):
        ...
        # Determine link state from port flags
        ...
        if new_state == "DOWN" and prev_state == "UP":
            ...
            self._write_alert({"type": "PORT_DOWN", **log_entry})
            ...
            self._write_alert({"type": "PORT_UP", **log_entry})
        ...
        # Log full entry to events log
        self._write_log(log_entry)

        # Flush MAC table entries for a downed port
        if new_state == "DOWN":
    ...
    def _monitor_loop(self):
        ...
        hub.sleep(5)  # Initial delay — let switches connect first
        while True:
    ...
    def port_stats_reply_handler(self, ev):
        ...
        stats_file = os.path.join(LOG_DIR, f"stats_{dpid:#x}.json")
    ...
    def switch_disconnect_handler(self, ev):
        ...
        # Remove from datapaths so stats loop doesn't try to poll it
    ...
    def _write_log(self, entry):
        ...
        with open(os.path.join(LOG_DIR, "events.json"), "a") as f:
